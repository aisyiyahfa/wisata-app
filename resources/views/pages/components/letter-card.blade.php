        ...
        <div class="d-flex justify-content-between flex-column flex-sm-row">
            ...
            <div class="card-title d-flex flex-row">
                ...
                @if($letter->type == 'incoming')
                    <div class="mx-3">
                        <a href="{{ route('sekretaris.disposition.index', $letter) }}"
                ...
                <div class="dropdown d-inline-block">
                        ...
                        <div class="dropdown-menu dropdown-menu-end"
                            ...
                            @if(!\Illuminate\Support\Facades\Route::is('*.show'))
                                <a class="dropdown-item"
                                   href="{{ route('sekretaris.incoming.show', $letter) }}">{{ __('menu.general.view') }}</a>
                            @endif
                            <a class="dropdown-item"
                               href="{{ route('sekretaris.incoming.edit', $letter) }}">{{ __('menu.general.edit') }}</a>
                            <form action="{{ route('sekretaris.incoming.destroy', $letter) }}" class="d-inline"
                                  method="post">
                                @csrf
                                @method('DELETE')
                                <span
                    ...
                    @else
                        <div class="dropdown-menu dropdown-menu-end"
                            ...
                            @endif
                            <a class="dropdown-item"
                               href="{{ route('sekretaris.outgoing.edit', $letter) }}">{{ __('menu.general.edit') }}</a>
                            <form action="{{ route('sekretaris.outgoing.destroy', $letter) }}" class="d-inline"
                                  method="post">
                                @csrf
                                @method('DELETE')
                                <span
